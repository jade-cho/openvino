...
TEST(update_shape_test, paged_attention_mixed_stage_token_type_ids_disables_micro_layout) {
    ...
    // as the wrong buffer count. token_type_ids forces the non-micro path in MIXED, so the count is
    // the partition-kernel one (7) on every device, including micro-kernel-capable ones.
